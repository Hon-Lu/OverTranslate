<#
    檢查發布產物裡那兩顆未簽章原生檔的 SHA256 有沒有變。

    免安裝包根目錄的啟動器 stub 與 Update.exe 都沒有數位簽章，而 Defender 的雲端信譽是
    **按檔案雜湊**累積的（見 #210）。雜湊一換，信譽就從零開始，那幾天最容易被報
    Trojan:Win32/Wacatac.B!ml。`--stableStub` 讓 stub 不再跟著版號與 commit 變，但圖示、
    app.manifest、組件資訊字串、vendor 二進位這幾項只要改了，它照樣會重算。
...
#                 vendor 的 stub.exe（換 vpk 版本就會換）
#   Update.exe  ← 應用程式圖示、vendor 的 update.exe
# 版號與 commit 不在上面任何一條裡，那正是 --stableStub 凍掉的東西。
$expected = [ordered]@{
    # 第一顆 --stableStub 產物，2026-09-22 在本機量的。導入前每次發版都不一樣（8 次建置 8 顆）。
    $MainExe     = "39ffa5ee4600edfbb7a677c1e5da3bfb3a2bb571a6db29a4926617f28bcedac0"
    # 2.2.1-beta.2（2026-09-08 換 icon）起就沒變過，與版號無關。
...
Write-Host ""
Write-Host "免安裝包根目錄那兩顆未簽章原生檔：" -ForegroundColor Cyan
